...
# ── base64 이미지 ─────────────────────────────────
BASE64_IMAGES = {
    "rocket": "iVBORw0KGgoAAAANSUhEUgAAAHYAAAE7CAYAAAAIFnUXAAAYBElEQVR42u2d2XMU1/XH+Q9S/gsoqlwI"
    ...
    "adventurer": "iVBORw0KGgoAAAANSUhEUgAAAFAAAABuCAYAAACwYZRwAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFn"
    ...
    "stone": "iVBORw0KGgoAAAANSUhEUgAAAEYAAABGCAYAAABxLuKEAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFn"
    ...
    "AAAAAElFTkSuQmCC",
    "sword": "iVBORw0KGgoAAAANSUhEUgAAAEYAAABGCAYAAABxLuKEAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFn"
    "ZVJlYWR5ccllPAAAAyFpVFh0WE1MOmNvbS5hZG9iZS54bXAAAAAAADw/eHBhY2tldCBiZWdpbj0i77u/"
    "IiBpZD0iVzVNME1wQ2VoaUh6cmVTek5UY3prYzlkIj8+IDx4OnhtcG1ldGEgeG1sbnM6eD0iYWRvYmU6"
    "bnM6bWV0YS8iIHg6eG1wdGs9IkFkb2JlIFhNUCBDb3JlIDUuNS1jMDE0IDc5LjE1MTQ4MSwgMjAxMy8w"
    "My8xMy0xMjowOToxNSAgICAgICAgIj4gPHJkZjpSREYgeG1sbnM6cmRmPSJodHRwOi8vd3d3LnczLm9y"
...
def load_sprite(name):
    try:
        ...
        pygame.draw.rect(surf, (255, 0, 255), (0, 0, 100, 100), 3)
        pygame.draw.line(surf, (255, 0, 255), (0, 0), (100, 100), 2)
        pygame.draw.line(surf, (255, 0, 255), (100, 0), (0, 100), 2)
        return surf
...
def project(cx, cy, w, h, angle, axis):
    a = math.radians(angle)
    hw, hh = w/2, h/2
    corners = [
    ...
    dots = [cx*axis[0] + cy*axis[1] + c[0]*axis[0] + c[1]*axis[1] for c in corners]
    return min(dots), max(dots)

def obb_collide(cx1, cy1, w1, h1, a1, cx2, cy2, w2, h2, a2):
    axes = get_axes(a1) + get_axes(a2)
    for axis in axes:
        mn1, mx1 = project(cx1, cy1, w1, h1, a1, axis)
        mn2, mx2 = project(cx2, cy2, w2, h2, a2, axis)
...
class GameObject:
    ...
    def set_sprite(self, name):
        self.name = name
        img = fit_surface(load_sprite(name), *SPRITE_SIZE)
        self.base = img
        self.image = img
    ...
    def radius(self, mode):
        if mode == "min": return min(self.w, self.h)/2
        if mode == "max": return max(self.w, self.h)/2
    ...
    def draw(self, screen):
        r = self.image.get_rect(center=(self.x, self.y))
        screen.blit(self.image, r)

    def draw_bb(self, screen, mode, rmode, highlight=False):
        color = (255,255,255)
        if highlight:
            color = (255,255,0)  # ⭐ 충돌 순간 강조

        if mode == "circle":
            pygame.draw.circle(screen, color, (int(self.x), int(self.y)), int(self.radius(rmode)), 3)
        elif mode == "aabb":
            ...
            corners = [
                ( self.x + hw*math.cos(a)-hh*math.sin(a), self.y + hw*math.sin(a)+hh*math.cos(a)),
                ( self.x - hw*math.cos(a)-hh*math.sin(a), self.y - hw*math.sin(a)+hh*math.cos(a)),
                ( self.x - hw*math.cos(a)+hh*math.sin(a), self.y - hw*math.sin(a)-hh*math.cos(a)),
                ( self.x + hw*math.cos(a)+hh*math.sin(a), self.y + hw*math.sin(a)-hh*math.cos(a)),
            ]
            pygame.draw.polygon(screen, color, corners, 3)

# ── 메인 ─────────────────────────────────
def main():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("Collision Demo FINAL++")
    ...
    player = GameObject(200, 300, SPRITE_NAMES[0])
    enemy = GameObject(600, 300, SPRITE_NAMES[0])
    ...
    while running:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                ...
                if e.key == pygame.K_b: bb_idx = (bb_idx+1)%3
                if e.key == pygame.K_m: radius_idx = (radius_idx+1)%3
                if e.key == pygame.K_s: rotating = not rotating
        ...
        rmode = RADIUS_MODES[radius_idx]

        # 충돌 계산
        if mode == "circle":
            hit = circle_collide(player.x, player.y, player.radius(rmode),
                                 enemy.x, enemy.y, enemy.radius(rmode))
        elif mode == "aabb":
            hit = aabb_collide(player.rect, enemy.rect)
        else:
            hit = obb_collide(player.x, player.y, player.w, player.h, player.angle,
                              enemy.x, enemy.y, enemy.w, enemy.h, enemy.angle)

        just_hit = hit and not prev_hit  # ⭐ 핵심

        screen.fill(BG_NORMAL)

        player.draw(screen)
        enemy.draw(screen)

        # ⭐ 충돌 순간만 강조
        player.draw_bb(screen, mode, rmode, highlight=just_hit)
        enemy.draw_bb(screen, mode, rmode, highlight=just_hit)

        # HUD
        hud = [
            f"Mode: {mode}",
            f"Collision: {'O' if hit else 'X'}",
            f"Sprite: {SPRITE_NAMES[sprite_idx]}",
            f"Rotate: {'ON' if rotating else 'OFF'}",
            f"Radius: {rmode}"
        ]
        ...
        for i, t in enumerate(hud):
            color = COLOR_TEXT
            if "Collision" in t:
                color = COLOR_HIT if hit else (180,180,180)
        ...
        for i, t in enumerate(controls):
            txt = font.render(t, True, (180,180,180))
            screen.blit(txt, (WIDTH-200, 10 + i*20))
